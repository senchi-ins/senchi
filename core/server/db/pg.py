...
class PostgresDB:
    ...
    def get_properties(self, user_id: str) -> Optional[List[str]]:
        query = """
        SELECT id, name, address, property_type, description, scores_overall, scores_internal, scores_external, devices_connected, devices_total, alerts FROM zb_properties WHERE id IN (SELECT property_id FROM zb_user_properties WHERE user_id = %s)
        """
